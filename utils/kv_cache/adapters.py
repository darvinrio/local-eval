...
class StandardGQAAdapter(KVCacheAdapter):
    ...
    def estimate(
        ...
        num_layers = text_config.get("num_hidden_layers", text_config.get("num_layers", 0))
        num_kv_heads = text_config.get("num_key_value_heads", text_config.get("num_attention_heads", 0))
        head_dim = text_config.get("head_dim", 0)
        
        if head_dim == 0 and "hidden_size" in text_config and "num_attention_heads" in text_config:
        ...
        if bpt == 0:
            logger.warning("KV cache estimate is 0. Config may be missing expected keys.")
...
class Gemma4Adapter(KVCacheAdapter):
    ...
    def estimate(
        ...
        breakdowns = []
        if num_full > 0:
            breakdowns.append(LayerKVCacheInfo(
...
class Qwen36HybridAdapter(KVCacheAdapter):
    ...
    def estimate(
        ...
        if num_linear > 0:
            ...
            fixed_state_bytes = int((linear_num_key_heads * linear_key_head_dim + linear_num_value_heads * linear_value_head_dim) * dtype_bytes)
...
def get_adapter(model_type: str) -> KVCacheAdapter:
    ...
    if cls is StandardGQAAdapter and model_type:
        logger.warning(f"No specific adapter for {model_type}, falling back to StandardGQAAdapter.")
    return cls()
